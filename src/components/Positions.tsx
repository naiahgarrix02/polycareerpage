  ...
  return (
    <div className="lg:px-40 md:px-12 px-4">
      ...
      <div className="flex flex-col gap-9">
        {postitions.map((post) => (
          <Link
            to={`/positions/${post.id}`}
            key={post.title}
            className="block no-underline"
          >
            <div className="group flex md:flex-row flex-col lg:gap-9 md:gap-20 gap-14 lg:p-12 md:p-8 p-4 justify-between cursor-pointer hover:translate-y-1 hover:bg-[#84143C] transition-colors duration-250 ease-out border-b border-[#a5a5a5]">
              <div className="flex flex-col gap-9">
              ...
              <div className="flex flex-row items-start md:gap-4 gap-2 transform transition-all duration-300 group-hover:translate-x-2">
                <p className="font-manrope-semibold lg:text-[32px] text-[24px] text-black group-hover:text-white">
                  Apply
                </p>
                <img
                  src="/arrowup.svg"
                  alt="arrow"
                  className="group-hover:invert w-8 h-8 lg:w-12 lg:h-12"
                />
              </div>
            </div>
          </Link>
